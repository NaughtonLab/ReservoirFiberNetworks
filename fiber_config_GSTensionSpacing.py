...
if __name__ == '__main__':
    ...
    parent_folder = "Simulations/SAGE/GridSearch/Tension_Spacing"
    grid = np.load(f'{parent_folder}/tension_spacing_force_grid_new.npz', allow_pickle=True)
    grid = grid['grid']

    parser = argparse.ArgumentParser(description="Script that configures and launches fiber_simulation for Tension-Spacing Grid Search")
    ...
    print("grid idx", grid_idx, "tension", tension_force, "force", point_force_mag, "spacing", spacing*1e3, "length", thread_length*1e3, "dx", dx*1e3, "n_elem", n_elem)
    ...
    params = {
        ...
        'tension_force': tension_force, # 1 N = kg m/s2 --> 1e6 g mm/s2 --> 1e3 mg mm /ms2  
