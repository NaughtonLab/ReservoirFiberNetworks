...
if __name__ == '__main__':
    parent_folder = "Simulations/SAGE/ThreadSpacing"
    
    grid = np.load(f'{parent_folder}/thread_spacing_grid_MC.npz', allow_pickle=True)
    grid = grid['grid']

    parser = argparse.ArgumentParser(description="Script that configures and launches fiber_simulation for Thread Spacing grid search on best Memory capacity")
    ...
    print("grid idx", grid_idx, "force", point_force_mag, "spacing", spacing*1e3, "num_threads", num_horizontal_threads, "dx", dx*1e3)
