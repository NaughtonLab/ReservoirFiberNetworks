...
from utils.boundary_conditions.constraints import DisplacementBC
...
class fiber_simulation():
    ...
    def add_threads(self):
        ...
        for i in range(self.num_horizontal_threads):
            ...
            self.simulator.constrain(self.horizontal_thread[i]).using(
                ...
                displacement_magnitude=self.point_force_mag/1e6
            )
    ...
    def launch_sim(self):
        ...
        name = f"test3_{self.num_horizontal_threads}by{self.num_vertical_threads}rods_{suffix}"
        print(name)
