...
if __name__ == '__main__':
    ...
    grid = np.load(f'{parent_folder}/force_spacing_grid.npz', allow_pickle=True)
    grid = grid['grid']
    ...
    print("grid idx", grid_idx, "force", point_force_mag, "spacing", spacing*1e3, "length", polygon_diameter*1e3, "dx", dx*1e3, "n_elem", n_elem)
    ...
    params = {
        ...
        'polygon_diameter': polygon_diameter, # 1 m --> 1e3 mm
        ...
        'point_force_mag': -point_force_mag, #-0.2 # 1 N = kg m/s2 --> 1e6 g mm/s2 --> 1e3 mg mm /ms2 
