...
class fiber_simulation():
    ...
    def add_threads(self):
        ...
        for i in range(len(self.radial_threads)):
            ...
            radial_thread_start = self.network_origin - 0.5 * self.polygon_diameter * radial_thread_direction
            if radial_thread_start[0] < 1e-10 and radial_thread_start[0] > -1e-10:
                radial_thread_start[0] = 0.0
            if radial_thread_start[1] < 1e-10 and radial_thread_start[1] > -1e-10:
                radial_thread_start[1] = 0.0
            if radial_thread_start[2] < 1e-10 and radial_thread_start[2] > -1e-10:
                radial_thread_start[2] = 0.0

            # print(radial_thread_start)
            # # radial_thread_start[0] = np.round(radial_thread_start[0], decimals=1)
            # # radial_thread_start[1] = np.round(radial_thread_start[1], decimals=1)
            # # radial_thread_start[2] = np.round(radial_thread_start[2], decimals=1)
            # # radial_thread_direction = np.array([np.cos(2 * np.pi * (i+1) / self.num_sides_polygon - offset), np.sin(2 * np.pi * (i+1) / self.num_sides_polygon - offset), 0.0])
            # # radial_thread_direction /= np.linalg.norm(radial_thread_direction)

            # radial_thread_direction = self.network_origin - radial_thread_start
            # radial_thread_direction /= np.linalg.norm(radial_thread_direction)

            # radial_thread_normal = np.cross(self.normal, radial_thread_direction)
            # radial_thread_normal /= np.linalg.norm(radial_thread_normal)
        ...
        for i in range(self.num_diagonal_threads):
            ...
            ang_1 = i * ang_between_radial_threads
            vertex_1 = 0.5 * self.polygon_diameter * np.array([np.cos(ang_1), np.sin(ang_1), 0.0]) + self.network_origin

            ## coordinates of next to next vertex of the polygon
            ang_2 = (i + 2) * ang_between_radial_threads
            vertex_2 = 0.5 * self.polygon_diameter * np.array([np.cos(ang_2), np.sin(ang_2), 0.0]) + self.network_origin
        ...
        ### Connections between diagonal threads and radial threads
        self.simulator.connect(self.diagonal_threads[0], self.radial_threads[1],
                               int(self.diag_to_rad_connect_idx), int(self.rad_to_diag_connect_idx_near_end)).using(FixedJoint,
                                                                                                         k = self.k,
                                                                                                         nu = self.nu,
                                                                                                         kt = self.kt)
        
        self.simulator.connect(self.diagonal_threads[1], self.radial_threads[2],
                               int(self.diag_to_rad_connect_idx), int(self.rad_to_diag_connect_idx_near_end)).using(FixedJoint,
                                                                                                         k = self.k,
                                                                                                         nu = self.nu,
                                                                                                         kt = self.kt)
        
        self.simulator.connect(self.diagonal_threads[2], self.radial_threads[0],
                               int(self.diag_to_rad_connect_idx), int(self.rad_to_diag_connect_idx_near_start)).using(FixedJoint,
                                                                                                         k = self.k,
                                                                                                         nu = self.nu,
                                                                                                         kt = self.kt)
        
        self.simulator.connect(self.diagonal_threads[3], self.radial_threads[1],
                               int(self.diag_to_rad_connect_idx), int(self.rad_to_diag_connect_idx_near_start)).using(FixedJoint,
                                                                                                         k = self.k,
                                                                                                         nu = self.nu,
                                                                                                         kt = self.kt)
        
        self.simulator.connect(self.diagonal_threads[4], self.radial_threads[2],
                               int(self.diag_to_rad_connect_idx), int(self.rad_to_diag_connect_idx_near_start)).using(FixedJoint,
                                                                                                         k = self.k,
                                                                                                         nu = self.nu,
                                                                                                         kt = self.kt)
        
        self.simulator.connect(self.diagonal_threads[5], self.radial_threads[0],
                               int(self.diag_to_rad_connect_idx), int(self.rad_to_diag_connect_idx_near_end)).using(FixedJoint,
                                                                                                         k = self.k,
                                                                                                         nu = self.nu,
                                                                                                         kt = self.kt)
        
        ### Connections between diagonal threads and diagonal threads
        self.simulator.connect(self.diagonal_threads[0], self.diagonal_threads[1],
                               int(self.diag_diag_connect_idx_near_end), int(self.diag_diag_connect_idx_near_start)).using(FixedJoint,
                                                                                                         k = 1e-2*self.k,
                                                                                                         nu = self.nu,
                                                                                                         kt = self.kt)
        self.simulator.connect(self.diagonal_threads[1], self.diagonal_threads[2],
                               int(self.diag_diag_connect_idx_near_end), int(self.diag_diag_connect_idx_near_start)).using(FixedJoint,
                                                                                                         k = 1e-2*self.k,
                                                                                                         nu = self.nu,
                                                                                                         kt = self.kt)
        self.simulator.connect(self.diagonal_threads[2], self.diagonal_threads[3],
                               int(self.diag_diag_connect_idx_near_end), int(self.diag_diag_connect_idx_near_start)).using(FixedJoint,
                                                                                                         k = 1e-2*self.k,
                                                                                                         nu = self.nu,
                                                                                                         kt = self.kt)
        self.simulator.connect(self.diagonal_threads[3], self.diagonal_threads[4],
                               int(self.diag_diag_connect_idx_near_end), int(self.diag_diag_connect_idx_near_start)).using(FixedJoint,
                                                                                                         k = 1e-2*self.k,
                                                                                                         nu = self.nu,
                                                                                                         kt = self.kt)
        self.simulator.connect(self.diagonal_threads[4], self.diagonal_threads[5],
                               int(self.diag_diag_connect_idx_near_end), int(self.diag_diag_connect_idx_near_start)).using(FixedJoint,
                                                                                                         k = 1e-2*self.k,
                                                                                                         nu = self.nu,
                                                                                                         kt = self.kt)
        self.simulator.connect(self.diagonal_threads[5], self.diagonal_threads[0],
                               int(self.diag_diag_connect_idx_near_end), int(self.diag_diag_connect_idx_near_start)).using(FixedJoint,
                                                                                                         k = 1e-2*self.k,
                                                                                                         nu = self.nu,
                                                                                                         kt = self.kt)
    ...
    def launch_sim(self):
        ...
        name = "radial_diagonal_threads_test_with_connections_and_force"
        print(name)
