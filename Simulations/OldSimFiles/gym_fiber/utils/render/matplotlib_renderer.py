...
class ElasticaRod(Geom):
    ...
    def __init__(self, rod, ax, rgb_color):
        self.rod = rod
        self.ax = ax
        ...
        self.scatter = ax.scatter(pos[0,:], pos[1,:], pos[2,:], s=convert_marker_size(rad, ax), c=self.rgb_color)
...
class Session(BaseElasticaRendererSession, BaseRenderer):
    ...
    def add_rod(self, rod, rod_type):
        if rod_type == 'rad':
            rgb_color = np.array([[0.35, 0.29, 1.0]])
        elif rod_type == 'bound':
            rgb_color = np.array([[1.0, 0.35, 0.29]])
        elif rod_type == 'spiral':
            rgb_color = np.array([0.0, 0.0, 0.0])
        else:
            print('Specify rod type')
        self.object_collection.append(ElasticaRod(rod, self.ax, rgb_color))
