...
class fiber_simulation():
    ...
    def launch_sim(self):
        ...
        suffix = f'tf{self.tf:.0e}_L{self.thread_length:.0e}_R{self.thread_radius:.2e}_YM{self.youngs_modulus:.2e}_D{self.damping_constant}_dx{self.dx}_pf{self.point_force_mag:.0e}' #'sweep_{self.max_episode_final_time:.0f}sec_{self.sample_freq:.0f}hz_d{self.damping_constant:.0f}_tension{self.tf:.0e}_pointforce{self.point_force_mag:.0e}_k{self.k:.0e}_kt{self.kt:.0e}_fps{self.rendering_fps}'
        name = f"{self.num_horizontal_threads+self.num_vertical_threads}rods_{suffix}_newunits_nocontacts" #_multiplevib"
        print(name)
        # 4rods_sweep_10sec_5hz_d10_tension1e+04_pointforce-2e+05_k1e+09_kt1e+09_fps250
...
def wrapper_launcher(params):
    num_horizontal_threads, num_vertical_threads, network_origin, thread_length, thread_diameter, dx, youngs_modulus, density, damping_constant, k, nu, kt, tf, point_force_mag, sample_freq, max_episode_final_time, rendering_fps, vib_thread_idx, sim_dt = params
    print(params)
    try:
...
if __name__ == '__main__':
    # original params               length=1000 mm, dia=4 mm,       dx=20 um,   E=230e6 Pa, damping=10, k=1e9,  nu=0,   kt=1e9, TF=1e4 uN,  PF=-2e5 uN                  sim_dt=5e-6

    # params in macro experiment -  length=500 mm,  dia=0.45 mm,    dx=?,       E=1.4e9 Pa, damping=?,  k=?,    nu=?,   kt=?,   TF=0.7 N, PF=?

    #                               assuming
    # params in micro experiment -  length=500 um,  dia=800 nm,    dx=?,        E=970e6 Pa, damping=?,  k=?,    nu=?,   kt=?,   TF=?, PF=?      
    
    # changed params 1 -            length=500 mm,  dia=0.45 mm,    dx=10 um,   E=5e6 Pa,   damping=10, k=1e9,  nu=0,   kt=1e9, TF=1e4 uN,  PF=-50 uN   (no contacts)   sim_dt=5e-6
    
    num_horizontal_threads = 2
    num_vertical_threads = num_horizontal_threads
    network_origin = np.zeros((3,))

    thread_length = 50e-2 * 1e3 # mm
    thread_diameter = 0.45e-3 * 1e3 # mm
    dx = 10 # mm

    youngs_modulus = 5e6 * 1e-3 # mg/mm ms2

    density = 1e3 * 1e-3 # mg/mm3
    ...
    tf = 1e4 * 1e3 # mg mm / ms2 or mN

    point_force_mag = -5e2 *1e3 # mN

    sample_freq = 5
    max_episode_final_time = 1 * 1e3 # ms
    rendering_fps = 250
