...
def get_data_from_file(name, k):
    if k == None:
        file = f"processing_data/{name}.pickle"
    else:
    ...
    # here stop is 4000 to get rid of NaN values
    stop = 4000 #len(force_profile)
    ...
    output = output[start:] # displacements - x(t)
    input = input[start:] # force - u(t)
    time = time[start:]

    return input, output, time

def post_processing_rod_data(name, sample_freq, n_files):

    name = name

    if n_files == 0:
        k = None
    capacity = np.empty((4, n_files), dtype=list)
    test_size = 0.75
    ...
    for k in range(1, n_files+1):
        ip, op, t = get_data_from_file(name, k)
    ...
    capacity_list_1_train_LinReg = []
    capacity_list_1_test_LinReg = []
    capacity_list_1_train_RidReg = []
    capacity_list_1_test_RidReg = []
    n_max = 100
    for n in range(n_max+1):
        ...
        clf_LinReg.fit(x_train, z_train)

        # Training capacity
        z_hat_train = clf_LinReg.predict(x_train)

        MSE = (1/len(z_train)) * np.sum((z_hat_train - z_train)**2)
        z2 = (1/len(z_train)) * np.sum((z_train)**2)
        C = 1 - MSE/z2

        capacity_list_1_train_LinReg.append(C)

        # Testing capacity
        z_hat = clf_LinReg.predict(x_test)

        MSE = (1/len(z_test)) * np.sum((z_hat - z_test)**2)
        z2 = (1/len(z_test)) * np.sum((z_test)**2)
        C = 1 - MSE/z2

        capacity_list_1_test_LinReg.append(C)

        ### Ridge Regression
        clf_RidReg = Ridge()
        clf_RidReg.fit(x_train, z_train)

        # Training capacity
        z_hat_train = clf_RidReg.predict(x_train)

        MSE = (1/len(z_train)) * np.sum((z_hat_train - z_train)**2)
        z2 = (1/len(z_train)) * np.sum((z_train)**2)
        C = 1 - MSE/z2

        capacity_list_1_train_RidReg.append(C)

        # Testing capacity
        z_hat = clf_RidReg.predict(x_test)

        MSE = (1/len(z_test)) * np.sum((z_hat - z_test)**2)
        z2 = (1/len(z_test)) * np.sum((z_test)**2)
        C = 1 - MSE/z2

        capacity_list_1_test_RidReg.append(C)

    '''Case 2: Reproduce product of current and n-steps old input'''
    # z(t) = u(t)*u(t-1) --> x = output[1:]; z = input[1:]*input[:-1]
    capacity_list_2_train_LinReg = []
    capacity_list_2_test_LinReg = []
    capacity_list_2_train_RidReg = []
    capacity_list_2_test_RidReg = []
    n_max = 100
    # for n in range(1, n_max+1):
        ...
        # capacity_list_2_train_LinReg.append(C)
        ...
        # capacity_list_2_test_LinReg.append(C)
        ...
        # capacity_list_2_train_RidReg.append(C)
        ...
        # capacity_list_2_test_RidReg.append(C)
    ...
    capacity_list_3_train_LinReg = []
    capacity_list_3_test_LinReg = []
    capacity_list_3_train_RidReg = []
    capacity_list_3_test_RidReg = []
    n_max = 100
    # for n in range(2, n_max+1):
        ...
        # capacity_list_3_train_LinReg.append(C)
        ...
        # capacity_list_3_test_LinReg.append(C)
        ...
        # capacity_list_3_train_RidReg.append(C)
        ...
        # capacity_list_3_test_RidReg.append(C)


    '''Case 4: Reproduce powers of the input'''
    # z(t) = u(t)^2 --> x = output; z = input**2
    # z(t) = u(t)^3 --> x = output; z = input**3
    capacity_list_4_train_LinReg = []
    capacity_list_4_test_LinReg = []
    capacity_list_4_train_RidReg = []
    capacity_list_4_test_RidReg = []
    n_max = 10
    # for n in range(1, n_max+1):
        # x = output
        # z = input**n
        ...
        # capacity_list_4_train_LinReg.append(C)
        ...
        # capacity_list_4_test_LinReg.append(C)
        ...
        # capacity_list_4_train_RidReg.append(C)
        ...
        # capacity_list_4_test_RidReg.append(C)
    ...
    capacity_list_5_train_LinReg = []
    capacity_list_5_test_LinReg = []
    capacity_list_5_train_RidReg = []
    capacity_list_5_test_RidReg = []
    n_max = 50
    # for n in range(n_max+1):
        # x = output
        # z = np.exp(n*input)
        ...
        # capacity_list_5_train_LinReg.append(C)
        ...
        # capacity_list_5_test_LinReg.append(C)
        ...
        # capacity_list_5_train_RidReg.append(C)
        ...
        # capacity_list_5_test_RidReg.append(C)
    ...
    capacity_list_6_train_LinReg = []
    capacity_list_6_test_LinReg = []
    capacity_list_6_train_RidReg = []
    capacity_list_6_test_RidReg = []
    n_max = 15
    for n in range(n_max+1):
        x = output
        ...
        capacity_list_6_train_LinReg.append(C)
        ...
        capacity_list_6_test_LinReg.append(C)
        ...
        capacity_list_6_train_RidReg.append(C)
        ...
        capacity_list_6_test_RidReg.append(C)

    if n_files > 0:
        capacity[0, k-1] = capacity_list_1_test_LinReg
        capacity[1, k-1] = capacity_list_2_test_LinReg
        capacity[2, k-1] = capacity_list_3_test_LinReg
        capacity[3, k-1] = capacity_list_4_test_LinReg
        capacity[4, k-1] = capacity_list_5_test_LinReg
        capacity[5, k-1] = capacity_list_6_test_LinReg
        print(f"Done file {k}")

    plt.subplot(2, 2, 1)
    # plt.plot(capacity[0, k-1], '-o', label = f"{k}")
    plt.plot(capacity_list_1_train_LinReg, '-o', label = 'Linear')
    ...
    plt.subplot(2, 2, 3)
    # plt.plot(capacity[1, k-1], '-o', label = f"{k}")
    plt.plot(capacity_list_1_test_LinReg, '-o', label = 'Linear')
    plt.plot(capacity_list_1_test_RidReg, '-o', label = 'Ridge')
    plt.xlabel("Number of Timesteps in the past")
    plt.ylabel("Capacity")
    plt.grid(True)
    ...
    # plt.subplot(3, 2, 3)
    # plt.plot(capacity[2, k-1], '-o', label = f"{k}")
    ...
    # plt.subplot(3, 2, 4)
    # plt.plot(capacity[3, k-1], '-o', label = f"{k}")
    ...
    plt.subplot(2, 2, 2)
    # plt.plot(capacity[4, k-1], '-o', label = f"{k}")
    plt.plot(capacity_list_6_train_LinReg, '-o', label = 'Linear')
    plt.plot(capacity_list_6_train_RidReg, '-o', label = 'Ridge')
    ...
    plt.subplot(2, 2, 4)
    # plt.plot(capacity[5, k-1], '-o', label = f"{k}")
    plt.plot(capacity_list_6_test_LinReg, '-o', label = 'Linear')
...
if __name__ == '__main__':
    ...
    post_processing_rod_data(name=name, sample_freq=sample_freq, n_files=n_files)
