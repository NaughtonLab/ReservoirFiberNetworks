...
if __name__ == '__main__':
    ...
    for spec in spec_list:
        ...
        plt.ylim(-0.1, 1.1)
        plt.grid()
        plt.savefig(f"{file_path}/{spec_label}_legendre_testing_plot_YM{YM}MPa.pdf", dpi=300)
        plt.close()

        plt.figure(figsize=(10, 5))
        plt.plot(np.linspace(0, max_time_back_seconds, max_timesteps_back+1), mem_capacity_test_list, label='Testing')
        plt.xlabel('Seconds in the Past')
        plt.ylabel('Capacity')
        plt.title('Memory Capacity')
        plt.ylim(-0.1, 1.1)
        plt.grid()
        plt.savefig(f"{file_path}/{spec_label}_memory_testing_plot_YM{YM}MPa.pdf", dpi=300)
        plt.close()
        ...
        # ax = plt.axes()
        # ax.matshow(capacity_test_matrix, cmap=matplotlib.colormaps['viridis'])
        # ax.set_title("Heatmap of Nonlinearity and Memory capacity")
        ...
        # # ax.set_xticks(np.arange(max_timesteps_back+1))
        # # ax.set_xticklabels(np.round(np.linspace(0, max_time_back_seconds, max_timesteps_back+1), 2))

        # for i in range(1, leg_max_order+1):
        #     for j in range(max_timesteps_back+1):
        #         c_test = capacity_test_matrix[i-1, j]
        #         ax.text(j, i-1, f"{c_test:.4f}", va='center', ha='center')

        # plt.savefig(f"{file_path}/{spec_label}_heatmap_testing_plot_YM{YM}MPa.pdf", dpi=300)
        # plt.close()

        np.savez(f"{file_path}/{spec_label}_legendre_memory_testing_YM{YM}MPa.npz",
                 leg_capacity_test_list=leg_capacity_test_list,
                 mem_capacity_test_list=mem_capacity_test_list)