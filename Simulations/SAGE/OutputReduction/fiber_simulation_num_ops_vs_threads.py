...
class fiber_simulation():
    ...
    def __init__(self, **kwargs):
        ...
        self.spacing = kwargs.get("spacing", 0.16666*1e3)
    ...
    def launch_sim(self):
        ...
        suffix = f'spacing{self.spacing:.4e}m_PF{self.point_force_mag/force_scale:.0e}N{self.TYPE_PF}_fps{self.rendering_fps}_stepskip{self.step_skip}'
        # name = f"{self.scaling_type}_FiberSim_{self.num_horizontal_threads+self.num_vertical_threads}rods_{suffix}"
        name = f"{self.num_horizontal_threads}by{self.num_vertical_threads}rods_{suffix}"
        print(name)
        ...
        for i in range(n_steps):
            ...
            if i%(100/self.sim_dt) == 0 and i != 0:
                ...
                np.savez(f"{self.loc}{name}_{i}steps.npz", rods_history=rods_history_temp)
